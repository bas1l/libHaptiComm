...
void Experiment::record_from_microphone() {
    ...
    while (!signal4stop_experiment())             // check if experiment is done
    {
        ...
        while (!signal4stop_experiment() && !signal4stop_recording() && i != nbReadMax) {
...
void Experiment::execute_stimuli() {
    ...
    std::pair<uint8_t, std::vector<uint16_t>> p;
    
    
    overruns = 0;
    length_motion_sub = 2000;
        ...
        for (int i=0; i<length_motion; i+=length_motion_sub) {
            ...
            if(local_waveform.begin()->second.begin()+i+length_motion_sub > local_waveform.begin()->second.end()) {
                for (it=local_waveform.begin(); it!=local_waveform.end(); ++it) {
                    ...
                    p = std::pair<uint8_t, std::vector<uint16_t>>(it->first, tmp_vector);
                    tmp_waveform.insert( p );
                }
            }
            else {
                for (it=local_waveform.begin(); it!=local_waveform.end(); ++it) {
                    std::vector<uint16_t> tmp_vector(it->second.begin()+i, it->second.begin()+i+length_motion_sub);
                    p = std::pair<uint8_t, std::vector<uint16_t>>(it->first, tmp_vector);
                    tmp_waveform.insert( p );
                }
            }
...
void Experiment::start_experiment() {
    ...
    case FingersSpace:
        ...
        break;
    case CalibrationWord:
        start_calibration_word();
        break;
    case CalibrationERM:
        start_calibration_ERM();
        break;
    default:
        std::cout
...
bool Experiment::start_calibration_word() {
    using namespace std::this_thread;
    using namespace std::chrono;
...
bool Experiment::start_calibration_ERM() {
    using namespace std::this_thread;
    using namespace std::chrono;
    std::cout << "[experiment][executeActuator] start..." << std::endl;
    // environmental variables
    ...
    int i, j, overruns, nb_sequences, rect;
    /* output variables */
    msec_array_t vhrc;
...
bool Experiment::start_space_actuator() {
    using namespace std::this_thread;
    using namespace std::chrono;
    std::cout << "[experiment][start_space_actuator] start..." << std::endl;

    /* (1/5)[MEMORY ALLOCATION] */
    // environmental variables
    waveformLetter valuestmp;
    
    int i, overruns, nb_sequences, rect;
    // output variables
    msec_array_t vhrc;
    msec_array_t timerDebug;
    int answeri, confidencei;
...
bool Experiment::start_space_finger() {
    using namespace std::this_thread;
    using namespace std::chrono;
    std::cout << "[experiment][start_space_finger] start..." << std::endl;
    // environmental variables
    waveformLetter valuestmp;
    bool rect;
    ...
    for (i = this->seq_start; i < nb_sequences; i++)   // for the sequence i
        ...
        start_recording();                    // start to record from microphone
        vhrc[0] = tool_now(this->c_start);          // get timing before stimuli
        std::cout << "[start_space_finger] timer since start=" << vhrc[0].count() << " ms"
                << std::endl;
        overruns += this->ad->execute_selective_trajectory(valuestmp,
                this->period_spi_ns); // execute the sequence
        this->ad->execute_trajectory(this->alph->getneutral(),
...
/*                                          */
/*------------------------------------------*/
bool Experiment::signal4waveform_ready() {
...
bool Experiment::signal4stoprecording() {
    std::unique_lock < std::mutex > mlock(this->m_mutex);
    this->m_condVar.wait(mlock,
            std::bind(&Experiment::is_recording_stoped, this));
    return true;
}
...
/*                                          */
/*------------------------------------------*/
int Experiment::read_answer(int * answeri) {
    int ret = 0;
    string ans;
    std::cout << "What was the answer?\t['s' = save and quit]\t['r' = restart]" << std::endl;
    cin >> ans;
    if (isdigit(ans[0])) {
        //std::cout<<"'"<<ans<<"' is a digit -> '"<<atoi(ans.c_str())<<"'"<<std::endl;
        *answeri = atoi(ans.c_str());
    } else if (0 == ans.compare("r")) { 
      ret = 2;
...
bool Experiment::is_recording_stoped() {
    return !(this->recording);
}
...
bool Experiment::is_waveformReady() {
    return this->waveformReady;
}
bool Experiment::is_executeMotion() {
    return this->executeMotion;
}
bool Experiment::is_executeMotion_stoped() {
    return !(this->executeMotion);
}
...
void Experiment::tool_save_results() {
  /* get the results */
  std::vector<msec_array_t> timers  = this->get_answer_timers();
  std::vector<int> answers          = this->get_answer_values();
  std::vector<int> confidence       = this->get_answer_confidences();
  std::vector<std::vector<int>> seq = this->get_actuators_sequences();
  std::vector<char> wfIDs           = this->get_waveforms_sequences();
  std::vector<int> wfIDs_int;
  wfIDs_int.reserve(wfIDs.size());
  for( char i : wfIDs ) wfIDs_int.push_back( i-'0' );
  
  int start = this->get_seq_start();
  int end = this->get_seq_end();
