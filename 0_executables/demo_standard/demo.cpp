...
int main(int argc, char *argv[]) {
   ...
   * 
   */
  extract_text = std::thread(workSymbols, std::ref(workdone), std::ref(alph),
      prosody);
  send_to_dac = std::thread(generateSentences, std::ref(workdone),
      alph->getlistSymbols());
...
void workSymbols(std::atomic<bool> & workdone, ALPHABET *& alph, int prosody) {
  ...
  while (!is_jobdone()) {
    ...
    if ((letters.front() == '+') || (letters.front() == '-')) {
      letters.pop_front();
      continue;
    } else if (prosody == PROSODY_SENTENCE
        && letters.end() != std::find(letters.begin(), letters.end(), '.')) {
      while (!letters.empty()) {
        if (letters.front() == ' ') {
          letters.pop_front();
          ...
          std::this_thread::sleep_for(
              std::chrono::milliseconds(PROSODY_SENTENCE_DELAY));
        } else {
