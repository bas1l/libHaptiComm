...
// thread functions
void generateSentences(std::atomic<bool> & workdone, std::string str_alph, char separator);
void workSymbols(std::atomic<bool> & workdone, ALPHABET* & alph, DEVICE * dev);

// work functions for thread functions
...
int main(int argc, char *argv[])
{
  ...
  do {
    ...
    send_to_dac = std::thread(workSymbols, std::ref(workdone), std::ref(alph), std::ref(dev));
    extract_text = std::thread( generateSentences, std::ref(workdone), alph->getlistSymbols(), prosody_separator);
...
void workSymbols(std::atomic<bool> & workdone, ALPHABET *& alph, DEVICE * dev)
{
  ...
  int duration_ms, delay_between_sequence_ms;
  ...
  delay_between_sequence_ms = PROSODY_WORD_DELAY;
  ...
  while(!is_jobdone())
  {
	...
	if (getWordActIDs(alph, &letters, &v))
	{
	  hb_driver.executeSymbol6by6(v, duration_ms, delay_between_sequence_ms);
	  v.clear();
...
bool getWordActIDs(ALPHABET *& alph, std::deque<char> * letters, std::vector<uint8_t> * output)
{
  ...
  // fill with all the input characters into a string
...
void init_window()
{
  ...
  printw("<F1> = Japanese Braille | <F5> = English Braille (letter rythm) | <F6> = English Braille (word rythm) | <F7> = English Braille (sentence rythm)\n");
