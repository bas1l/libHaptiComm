...
bool getWordActIDs(ALPHABET *& alph, std::deque<char> * letters,
    std::vector<uint8_t> * output);
bool getWordValue(ALPHABET *& alph, std::deque<char> * letters,
    waveformLetter * output);
    ...
    while (!letters.empty()) {
      if (letters.front() == ' ' || letters.front() == '.') {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(PROSODY_WORD_DELAY));
      } else {
        if (getWordActIDs(alph, &letters, &v)) {
          hb_driver.executeSymbol6by6(v, duration_ms,
              delay_between_sequence_ms);
          v.clear();
          std::this_thread::sleep_for(
              std::chrono::milliseconds(PROSODY_LETTER_DELAY));
        } else {
          letters.clear();
          std::cout << "?" << std::flush;
        }
