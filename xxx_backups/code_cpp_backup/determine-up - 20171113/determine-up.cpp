...
void workSymbols(std::queue<char> & sentences, std::condition_variable & cv, std::mutex & m, std::atomic<bool> & workdone){
    ...
    while(!workdone.load() or !sentences.empty())
    {
        ...
        if (personalSentences.front() != ' ')// is part of the alphabet){
        {
            ...
            push_letters(values, personalSentences.front(), apparent_movement, &freq_message);
...
//std::cout <<  && //std::cout << ++count << std::endl;
    ...
    std::thread consumer;
    //std::cout << "[consumer] before starting, joinable: " << consumer.joinable() << '\n';
    consumer = std::thread(workSymbols, std::ref(sentences), std::ref(cv),
                                            std::ref(m), std::ref(workdone));
    //std::cout << "[consumer] after starting, joinable: " << consumer.joinable() << '\n';
    consumer.join();
    
    
    std::thread producer;
    //std::cout << "[producer] before starting, joinable: " << producer.joinable() << '\n';
    producer = std::thread(generateSentences, std::ref(sentences), std::ref(cv),
                                            std::ref(m), std::ref(workdone));
    //std::cout << "[producer] after starting, joinable: " << producer.joinable() << '\n';
    producer.join();
